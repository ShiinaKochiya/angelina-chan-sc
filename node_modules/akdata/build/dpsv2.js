...
function getAliasedBlackboard(buffKey, blackboard, options) {
    var ret = Object.assign({}, blackboard);
...
class Character {
    setChar(char) {
        ...
        this.options = {};
        if (char.options)
            this.options = Object.assign({}, char.options);
    ...
    canResetAttack() {
        return (checkSpecs(this.skillId, "reset_attack") != false ||
            ["base_attack_time", "attack@max_target", "max_target"].some(x => this.blackboard[x] != null));
    }
}
const InitBuffFrame = {
    atk_scale: 1,
...
class AttributeFrames {
    constructor() {
        ...
        this.finalFrame = {};
        this.buffFrame = Object.assign({}, InitBuffFrame);
        this.applied = {};
        this.enemy = null;
    ...
    saveBuffs() {
        this.save = {
            buff: Object.assign({}, this.buffFrame),
            applied: Object.assign({}, this.applied)
        };
    }
    restoreBuffs() {
    ...
    calcFinalFrame() {
        ...
        let final = Object.assign({}, this.basicFrame);
        ...
        // 远程惩罚
        if (!buffs.no_ranged_penalty)
            final.atk *= buffs.ranged_penalty;
        // 防御
    ...
    calcDamagePool(atk = null, hitCount = 0, damageType = null) {
        ...
        if (this.crit)
            log = "[暴击] " + log;
        if (hitDamage < atk * this.buffFrame.minRate)
            log += " (抛光)";
...
class DpsContext {
    ...
    callSpecialForAny(funcKey, args, includeChar = false) {
        ...
        var list = Object.assign({}, this.buffList);
        if (includeChar)
            list[this.char.charId] = {};
        if (!args)
            args = {};
        for (var b in list) {
            if (b) {
    ...
    getDamageType() {
        if (!this.char.damageType)
            this.char.getDamageType();
    ...
    makeBuffList() {
        ...
        if (this.char.charData.trait)
            talents.unshift(this.char.charData.trait);
        talents.forEach(ta => {
        ...
        // 技能
        if (this.flags.skill)
            this.buffList[this.skillId] = this.char.blackboard;
        // 团辅
        if (this.options.buff)
            this.buffList["raidBuff"] = this.raidBlackboard;
    }
    ...
    writeBuff(text) {
        ...
        let line = [""];
        if (buffKey == this.skillId)
            line.push("[技能]");
        else if (buffKey == "raidBuff")
            line.push("[团辅/拐]");
        else
            line.push("[天赋]");
        if (checkSpecs(buffKey, "cond"))
            if (this.options.cond)
                ...
                line.push("[未触发]");
        if (checkSpecs(buffKey, "stack") && this.options.stack)
            line.push("[满层数]");
    ...
    applyBuffDefault(attr, buffKey, blackboard) {
        ...
        if (blackboard.max_stack_cnt) {
            ["atk", "def", "attack_speed", "max_hp"].forEach(key => {
                if (blackboard[key])
                    blackboard[key] *= blackboard.max_stack_cnt;
            });
        }
        // max_target spec
        ...
        // warmup prompt
        if (this.options.warmup && checkSpecs(buffKey, "multi_warmup"))
            this.log.note("多段暖机完成");
        // reflect prompt
        if (checkSpecs(buffKey, "reflect"))
            this.log.note("计算反射伤害，而非DPS");
        // original applyBuff
        for (var key in blackboard) {
            switch (key) {
                ...
                case "damage_scale":
                    ...
                    if (blackboard[key] != 1)
                        this.writeBuff(`${key}: ${blackboard[key].toFixed(2)}x`);
                    break;
    ...
    applyBuff(attr, buffKey) {
        var blackboard = Object.assign({}, this.buffList[buffKey]);
        ...
        if (!attr.applied[buffKey] && this.checkBuff(attr, buffKey)) {
            ...
            // 调用默认buff计算函数
            if (!this.retvar.done)
                this.applyBuffDefault(attr, buffKey, blackboard); // 参数都在this.args里
            attr.applied[buffKey] = true;
            this.log.popKey();
        }
    }
    ...
    calcRotation() {
        ...
        if (this.flags.skill) {
            ...
            if (this.callSpecialForAny("skill_duration", args, true)) {
                ...
                if (this.retvar.rotationFlags)
                    Object.assign(rotationFlags, this.retvar.rotationFlags);
            }
        ...
        if (this.flags.has_crit) {
            ...
            if (critAttackCount > 1)
                critAttackCount = Math.floor(critAttackCount);
    ...
    calcDamage() {
        ...
        for (var b in this.buffList) {
            ...
            if (!this.callSpecial(buffKey, "extraDamage", [args])) {
                ...
                if (_hps != 0)
                    this.writeBuff(`每秒恢复: ${_hps.toFixed(1)}`);
            }
            if (this.retvar.log)
                this.writeBuff(this.retvar.log);
    ...
    calcDps() {
        ...
        if (this.totalHeal != 0)
            this.log.write(`总治疗: ${this.totalHeal.toFixed(2)}`);
...
class DpsCalculator {
    ...
    explain() {
        var ret = {
            ...
            start: this.skill.rotation.startSp / (1 + this.normal.attr.buffFrame.spRecoveryPerSec),
            s_dps: this.skill.dps,
            s_hps: this.skill.hps,
            s_dmg: this.skill.totalDamage,
            s_heal: this.skill.totalHeal,
            s_atk: this.skill.attr.finalFrame.atk,
            s_dur: this.skill.rotation.duration,
            ...
            prep: this.skill.rotation.prepDuration,
            ...
            n_dur: this.normal.rotation.duration,
        ...
        ret.note = [];
        if (ret.s_log.note)
            ret.s_log.note.forEach(x => {
                if (!ret.note.includes(x))
                    ret.note.push(x);
            });
        if (ret.n_log.note)
            ret.n_log.note.forEach(x => {
                if (!ret.note.includes(x))
                    ret.note.push(x);
            });
        return ret;
...
export { Log, DpsContext, DpsCalculator };
