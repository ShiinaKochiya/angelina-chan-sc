...
function getCharAttributes(char) {
    checkChar(char);
    let { basic, buffs } = getAttributes(char, new Log());
...
function checkChar(char) {
    ...
    if (!('phase' in char))
        char.phase = charData.phases.length - 1;
    if (!('level' in char))
        char.level = charData.phases[char.phase].maxLevel;
    if (!('favor' in char))
        char.favor = 200;
    if (!('skillLevel' in char))
        char.skillLevel = 6;
    if (!('options' in char))
        char.options = { cond: true, crit: true, token: false };
    if (!('potentialRank' in char))
        char.potentialRank = 5;
}
class Log {
    constructor() {
        this.log = '';
...
function calculateDps(char, enemy, raidBuff) {
    ...
    if (char.skillLevel == -1)
        char.skillLevel = skillData.levels.length - 1;
    ...
    var _backup = {
        basic: Object.assign({}, attr.basic),
        //	enemy: {...enemy},
        //	chr: {...charData},
        //	level: {...levelData}
    };
...
function applyBuff(charAttr, buffFrm, tag, blackbd, isSkill, isCrit, log, enemy) {
    let buffFrame = __rest(buffFrm || initBuffFrame(), []);
    let blackboard = __rest(blackbd, []);
    ...
    function writeBuff(text) {
        ...
        if (checkSpecs(tag, "cond"))
            if (options.cond)
                ...
                line.push("[未触发]");
        if (checkSpecs(tag, "stack") && options.stack)
            line.push("[满层数]");
        // if (checkSpecs(tag, "crit")) line.push("[暴击]");
        if (checkSpecs(tag, "ranged_penalty"))
            line.push("[距离惩罚]");
    ...
    if (checkSpecs(tag, "cond")) { // 触发天赋类
        ...
        if (options.stack) { // 叠层天赋类
            if (tag == "tachr_2015_dusk_1" && options.token)
                done = true;
            else {
...
function checkResetAttack(key, blackboard) {
    if (checkSpecs(key, "reset_attack") == "false")
        return false;
    else
...
function calcDurations(isSkill, attackTime, attackSpeed, levelData, buffList, buffFrame, enemyCount, options, log) {
    ...
    if (checkSpecs(skillId, "sim")) {
        ...
        if (blackboard.ct)
            max_sp = spData.spCost * fps * blackboard.ct;
        if (blackboard.cnt)
            max_sp = spData.spCost * fps * blackboard.cnt;
        ...
        while (now <= duration * fps) {
            ...
            if (sp >= spData.spCost * fps &&
                time_since("skill") >= skill_time &&
                (time_since("attack") >= attackTime * fps || (checkSpecs(skillId, "attack_animation") && time_since("attack") == checkSpecs(skillId, "attack_animation")))) {
        ...
        if (isSkill) {
            ...
            if (rst) {
                ...
                if (!checkSpecs(skillId, "attack_begin"))
                    ...
                    log.writeNote(`技能前摇: ${t.toFixed(3)}s`);
            }
            // 技能类型
            if (levelData.description.includes("持续时间无限") || checkSpecs(skillId, "toggle")) {
                if (skillId == "skchr_thorns_3" && !options.warmup) { }
                    ...
                    if (checkSpecs(skillId, "toggle")) {
                        log.writeNote("切换类技能 (记为1800s)");
                        tags.push("toggle");
                    }
                    else {
                        log.writeNote("持续时间无限 (记为1800s)");
                        tags.push("infinity");
                    }
                }
            }
...
function calculateAttack(charAttr, enemy, raidBlackboard, isSkill, charData, levelData, log) {
    ...
    function calculateHitDamage(frame, scale) {
        ...
        if (buffList["tachr_144_red_1"])
            minRate = buffList["tachr_144_red_1"].atk_scale;
    ...
    for (var b in buffList) {
        ...
        switch (buffName) {
            ...
            case "tachr_346_aosta_1":
                ...
                if (blackboard.id == "skchr_aosta_2")
                    poison *= blackboard.talent_scale;
            ...
            case "tachr_391_rosmon_trait":
                var ntimes = 1;
                if (isSkill && blackboard.id == "skchr_rosmon_2")
                    ntimes = 3;
        ...
        if (bb["hp_ratio"]) {
            switch (buffName) {
                ...
                case "skchr_utage_2":
                case "skchr_akafyu_2":
                    pool[2] -= bb.hp_ratio * finalFrame.maxHp;
                    break;
        ...
        if (dmg > 0)
            log.write(`[特殊] ${displayNames[buffName]}: 额外伤害 ${dmg.toFixed(2)}`);
        if (pool[2] > 0)
            log.write(`[特殊] ${displayNames[buffName]}: 额外治疗 ${pool[2].toFixed(2)}`);
        else if (pool[2] < 0)
            log.write(`[特殊] ${displayNames[buffName]}: 自身伤害 ${pool[2].toFixed(2)}`);
        for (let i = 0; i < 5; ++i)
            extraDamagePool[i] += pool[i];
    }
    // 整理返回
    ...
    if (totalHeal != 0)
        log.write(`总治疗: ${totalHeal.toFixed(2)}`);
...
function calculateGradDamage(_) {
    ...
    if (_.charId == "char_328_cammou") {
        ...
        dmg_table = [...Array(_.dur.attackCount).keys()].map(x => (x >= tb.length ? Math.round(_.hitDamage * tb[tb.length - 1]) : Math.round(_.hitDamage * tb[x])));
...
export { getCharAttributes, calculateDps, calculateDpsSeries };
